...
public class DiagonalMoveset extends MoveStrategy
{
    ...
    private ArrayList<Tuple<Integer, Integer>> generateEntry(int origin_row, int origin_col, Board board)
    {
        ...
        // NE
        for (int i = 1; i < 8; i++) {
            Tuple<Integer, Integer> tmp = new Tuple<>(origin_row - i, origin_col + i);
            if (checkValidRange(tmp.getFirst(), tmp.getSecond())) {
                Block block = board.getBlockFromCoords(origin_row - i, origin_col + i);
                if (block.getPiece() != null) {
                    if (!block.getPiece().getColor().equals(color)) {
                        moves.add(new Tuple<>(origin_row - i, origin_col + i));
                    }
                    break;
                } else {
                    moves.add(new Tuple<>(origin_row - i, origin_col + i));
